...
if (typeof document !== 'undefined') (function () {
  ...
  async function loadTests() {
    const files = { t1: 't1_sfen.json', t2: 't2_usi.json', t3: 't3_fouls.json', t4: 't4_perft.json', t5: 't5_legalmoves.json', t6: 't6_tsume1.json' };
    const tests = {};
    for (const key of Object.keys(files)) {
      const r = await fetch(`tests/${files[key]}?v=${R.BUILD_JS}`, { cache: 'no-store' });
      if (!r.ok) throw new Error(`${files[key]} の読込失敗 (${r.status})`);
      tests[key] = await r.json();
    }
    return tests;
  }
  ...
  async function runAll() {
    ...
    try {
      ...
      const sum = `合計 PASS ${res.pass} / FAIL ${res.fail} (94件)`;
      ...
      log(`findAllMates: 最大${res.tsumeMax.toFixed(1)}ms / 平均${res.tsumeAvg.toFixed(1)}ms (目標: 最大50ms以下)`, res.tsumeMax <= 50 ? 'ok' : 'warn');
    } catch (e) {
