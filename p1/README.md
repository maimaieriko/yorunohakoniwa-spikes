...
## Change Log
### p1-007 (2026-07-10) 収録検証の3層化(全match問題へ一般化)
- **boardRefs**: 全6問に「解説が前提とする配置」を構造化宣言し、SFENと照合(誤記の直接検出)
- **semanticAsserts全match化**: ただ取り(取れる+取り返されない)/王手飛車/角道が開く(可動域増)/飛車先の歩
- **本文マス言及スキャン**: 文中の「3一」等を抽出し、占有・正解手のfrom/to・正解後の利きと整合するか検査
- **逆テスト実施**: 旧誤記(飛車7一)を仕込むと**boardRefs層が捕捉**することを確認。attacks意味検証は
  「マスへの利き」を見るため駒不在でも通過する=層の分担どおりboardRefsが誤記検出の主層であることを実証
- 監査結果: **S-NEXT-002以外の5問に同種の誤記なし**(全問3層通過)。規約§2に3層必須を明文化
- Node: PASS 145 / FAIL 0(収録検証が7→22チェックに増加)
### p1-006 (2026-07-10) S-NEXT-002の問題データ修正(実機報告対応)
- **真因**: 問題データのSFEN誤記。盤上の飛車が7一に居り、解説・正解(N*4c=王手飛車)は3一前提で
  不整合。どこに打っても不正解になる壊れた出題だった(判定系・USI伝達は正常=T8-17/18で証明)
- 修正: SFENを飛車3一へ(N*4cが王手+飛車取りになることを機械確認)
- **再発防止**: 収録検証に「意味的アサーション」を追加(次の一手の正解手が王手/対象マスへの利きを
  満たすかをsemanticAssertsで機械検証)。T8: 持ち駒打ちの単体2件追加(計23件、総129件)
- 判定ログ追加: 問題ID/実着手USI/correctMoves等の内訳/結果を毎判定で画面ログへ(subject.judgeが
  detailを返す契約の任意拡張として記録)
- 対応範囲はこの判定系のみ(P1-c実装は含まない)
