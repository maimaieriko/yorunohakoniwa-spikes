...
## 5. 変更履歴
### s3-002 (2026-07-09) テスト取得のフォールバック
- 実機報告: spike3/tests/t7_tsume3.json がGitHub上は正しい階層なのにPagesで404
- runner.js: t1〜t7の取得を「tests/<file> → 404なら同階層 <file>」の2段フォールバックに変更。
  **実際にfetchした絶対URLと結果(成功/HTTPステータス/ネットワークエラー)を全件ログ出力**
  → どのURLで何が起きたかが実機ログで確定できる
- 差し替え単位はtests/を除く9ファイル(tsume.html等のHTMLは ?v 付きでJSを読むため、
  HTMLを替えないと古いJSのキャッシュが残り得る+ビルドID全値一致の照合が赤警告になるため)
